  ...
  Annotator.Plugin.AnnotatorViewer = (function(_super) {
    ...
    function AnnotatorViewer(element, options) {
      ...
      jQuery("#annotations-panel").click(function(event) {
        jQuery(".container-anotacions").toggle("slide");
      });
    ...
    */
    ...
    AnnotatorViewer.prototype.textareaEditor = function(annotator_textArea,item) {
        ...
        if (jQuery('li#annotation-'+item.id).find('textarea.panelTextArea').length==0) {
          ...
          jQuery('<a href="#save" class="annotator-panel-save">Save</a>').appendTo(control_buttons).bind("click",{annotation:item},this.onSavePanel);
          jQuery('<a href="#cancel" class="annotator-panel-cancel">Cancel</a>').appendTo(control_buttons).bind("click", {annotation:item},this.onCancelPanel);
        }
    };
